...
public class NationalSavingsCertificate extends BaseInvestmentJob {

    public NationalSavingsCertificate(String accNo, String[] names, Double intDeposited, Double principal,
            String medium,
    ...
    public static void executeJob() {
        try {
            ArrayList<ExcelRow> rows = ExcelAPI
                    .readExcel(PropertiesReader.getExcelProperty("SHEET.NATIONAL_SAVINGS_CERTIFICATE"));
            ...
            for (ExcelRow row : rows) {
                if (calendar.get(Calendar.YEAR) == row.getMaturityYear()
                    ...
                    if (row.getNames().length > 1)
                        multipleUsers = true;
                    NationalSavingsCertificate job = new NationalSavingsCertificate(row.getAccNo(), row.getNames(),
    ...
    String generateNotificationString() {
        return String.format(
                "This is to remind you that your account number %s should be credited with Rs. %f today (%s) for the principal of Rs. %f in %s. This account is in the name of %s for National Savings Certificate.\n\nSent with concern,\nCRONUS.",
    ...
    private String generateMultipleUsersNotificationString() {
        return String.format(
                "This is to remind you that your account number %s should be credited with Rs. %f today (%s) for the principal of Rs. %f in %s. This account is in the name of %s and %s for National Savings Certificate.\n\nSent with concern,\nCRONUS.",
                this.getAccNo(), this.getIntDeposited(), this.getDate(), this.getPrincipal(), this.getMedium(),
